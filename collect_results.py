...
class ResultsCollector:
    ...
    def _generate_comparison_metrics(self, model_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        completed_models = [
            model for model, results in model_results.items() 
        ...
        if completed_models:
            ...
            # Identify best model
            comparison["best_model"] = self._identify_best_model(comparison["performance_comparison"])
        ...
        for model, metrics in model_metrics.items():
            for split, split_metrics in metrics.items():
                if isinstance(split_metrics, dict):
                    ...
                    # Clinical metrics
                    row["cost_weighted_accuracy"] = split_metrics.get("cost_weighted_accuracy", None)
    ...
    def generate_comprehensive_report(self) -> str:
            ...
            if status == "completed" and model_data["metrics"]:
                ...
                if "test" in model_data["metrics"]:
                    ...
                    if isinstance(test_metrics, dict):
                        ...
                        report_lines.append(f"  Test F1: {f1:.4f}, AUC: {auc:.4f}" if f1 != "N/A" else "  Metrics available")
        ...
        report_lines.extend([
            f"MLflow Experiments: mlruns/",
            f"Training Logs: logs/*_{self.session_timestamp}.log",
            f"Monitoring Output: monitor_output.log",
            ""
        ])
        ...
        report_lines.extend([
            "=" * 80,
            "END OF REPORT",
            f"Generated by Suicide Risk Detection Research Framework v1.0",
            f"For questions or issues, contact the research team.",
            "=" * 80
        ])
            ...
            if not test_data.empty:
                ...
                for i, model in enumerate(test_data["model"].unique()):
                    ...
                    if not model_data.empty:
                        values = [model_data[metric].iloc[0] if metric in model_data.columns and not pd.isna(model_data[metric].iloc[0]) else 0 
...
def main():
    ...
    completed_models = sum(
        1 for model_data in results["models"].values() 
        if model_data["status"] == "completed"
    )
    
    in_progress_models = sum(
        1 for model_data in results["models"].values() 
        if model_data["status"] == "in_progress"
    )
