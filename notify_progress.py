...
def check_training_status():
    ...
    status = {"timestamp": datetime.now().isoformat(), "models": {}}
    ...
    for model in models:
        ...
        try:
            ...
            status["models"][model] = {"error": str(e), "status": "error"}
...
def main():
    ...
    while True:
        try:
            ...
            _running_models = sum(
                1 for m in status["models"].values() if m.get("status") == "running"
            )
            completed_models = sum(
                1 for m in status["models"].values() if m.get("status") == "completed"
            )
