...
class TrainingWatchdog:
    ...
    def load_state(self) -> Dict:
        ...
        if os.path.exists(self.config_file):
            with open(self.config_file, "r") as f:
                return json.load(f)
        return {"processes": {}, "retry_counts": {}, "last_progress": {}}
    ...
    def restart_training(self, model: str, config: Dict) -> Optional[int]:
        ...
        cmd = [
            ...
            "kaggle",
            "--output_dir",
            f"results/model_outputs/{model}_mps",
            "--prefer_device",
            config.get("prefer_device", "mps"),
            "--config",
            "configs/mps.yaml",
        ]
    ...
    def kill_hung_mps_processes(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["pgrep", "-f", "MetalPerformanceShaders"], capture_output=True, text=True
            )
            ...
            try:
                for model, proc_info in list(self.state["processes"].items()):
                    ...
                    if proc_status is None:
                        ...
                        print(
                            f"⏸️  {model} appears to be stalled (no progress in {self.stall_threshold}s)"
                        )
