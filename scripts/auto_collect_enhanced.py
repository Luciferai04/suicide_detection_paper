...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/auto_collect_enhanced.log"), logging.StreamHandler()],
)
...
class ResultsCollector:
    def __init__(self):
        ...
        self.collected = set()
        self.results_file = (
            f"results/training_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        with open(report_file, "w") as f:
            ...
            for model_type, results in models.items():
                ...
                for result in sorted(results, key=lambda x: x.get("accuracy", 0), reverse=True):
                    ...
                    f.write(
                        f"  Device: {result.get('device', 'unknown')} ({result.get('device_name', 'unknown')})\n"
                    )
                ...
                cpu_result = next(
                    (r for r in results if "cpu" in r.get("device", "").lower()), None
                )
                mps_result = next(
                    (r for r in results if "mps" in r.get("device", "").lower()), None
                )

                if (
                    cpu_result
                    and mps_result
                    and "training_time" in cpu_result
                    and "training_time" in mps_result
                ):
    ...
    def trigger_visualization(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["python", "scripts/generate_charts.py"], capture_output=True, text=True, timeout=60
            )
            ...
            try:
                ...
                if len(self.collected) == len([d for d in self.output_dirs if Path(d).exists()]):
                    ...
                    if all_results:
                        ...
                        logger.info(
                            f"Best model: {best['model_name']} with accuracy {best.get('accuracy', 0):.4f}"
                        )
