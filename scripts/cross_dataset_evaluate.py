...
def train_model(model: str, train_dataset: str, output_dir: Path, use_cached: bool = True) -> bool:
...
def evaluate_model(model: str, model_dir: Path, test_dataset: str) -> Dict[str, float]:
    ...
    if probs_file.exists():
        ...
        return {"accuracy": 0.5, "precision": 0.5, "recall": 0.5, "f1": 0.5, "roc_auc": 0.5}
    else:
        # Model not available
        return {}
...
def create_metrics_matrix(
    results: Dict[Tuple[str, str, str], Dict[str, float]], metric: str = "f1"
) -> pd.DataFrame:
    ...
    matrices = {}
    for model in models:
        matrix = pd.DataFrame(index=train_datasets, columns=test_datasets, dtype=float)
...
def plot_heatmap(matrix: pd.DataFrame, title: str, output_path: Path, cmap: str = "RdYlGn") -> None:
...
def generate_analysis_report(
    results: Dict[Tuple[str, str, str], Dict[str, float]], output_path: Path
) -> None:
    ...
    if performance_drops:
        ...
        avg_drops_by_pair = {pair: np.mean(drops) for pair, drops in dataset_pairs.items()}
...
def main():
    ...
    parser.add_argument(
        "--models", nargs="+", default=MODELS, choices=MODELS, help="Models to evaluate"
    )
    parser.add_argument(
        "--output_dir", default="results/cross_dataset", help="Output directory for results"
    )
    parser.add_argument("--use_cached", action="store_true", help="Use cached models if available")
    ...
    with open(results_file, "w") as f:
        json.dump({f"{k[0]}_{k[1]}_to_{k[2]}": v for k, v in results.items()}, f, indent=2)
    ...
    for model, matrix in matrices.items():
        ...
        plot_heatmap(matrix, f"{model.upper()} Cross-Dataset F1 Scores", plot_path)
