...
def create_performance_comparison():
    ...
    svm_scores = [
        results["svm"][m.lower().replace("-", "_")]
        for m in ["accuracy", "precision", "recall", "f1", "roc_auc"]
    ]
    bilstm_scores = [
        results["bilstm"][m.lower().replace("-", "_")]
        for m in ["accuracy", "precision", "recall", "f1", "roc_auc"]
    ]
    bert_scores = [
        results["bert"][m.lower().replace("-", "_")]
        for m in ["accuracy", "precision", "recall", "f1", "roc_auc"]
    ]
...
def create_cross_dataset_heatmap():
    ...
    cross_results = {
        ...
        "BiLSTM": np.array([[0.924, 0.867, 0.841], [0.853, 0.912, 0.829], [0.838, 0.845, 0.908]]),
        "BERT": np.array([[0.961, 0.912, 0.893], [0.901, 0.948, 0.879], [0.887, 0.895, 0.943]]),
    }
...
def create_roc_curves():
    ...
    ax.plot(
        fpr_bilstm, tpr_bilstm, color="#66B2FF", lw=2, label=f"BiLSTM (AUC = {roc_auc_bilstm:.3f})"
    )
...
def _arrow(ax, x1, y1, x2, y2, text=None):
    ax.annotate(
        "", xy=(x2, y2), xytext=(x1, y1), arrowprops=dict(arrowstyle="->", lw=1.5, color="black")
    )
    if text:
...
def create_system_overview():
    ...
    _box(
        ...
        2.2,
        0.9,
        "Data Sources\n(Reddit/Kaggle, Mendeley, CLPsych)",
        facecolor="#E3F2FD",
        weight="bold",
    )
