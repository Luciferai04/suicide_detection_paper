...
class ErrorAnalyzer:
    ...
    def categorize_errors(self, texts: List[str], y_true: np.ndarray, y_pred: np.ndarray) -> Dict:
        ...
        for i, (true_label, pred_label) in enumerate(zip(y_true, y_pred)):
            ...
            if true_label == 1 and pred_label == 1:
                error_analysis["true_positives"].append(
                    {"index": i, "text": text, "confidence": None}
                )
                ...
                error_analysis["false_positives"].append(
                    {"index": i, "text": text, "category": category, "confidence": None}
                )
    ...
    def _categorize_false_positive(self, text: str) -> str:
        ...
        if any(
            word in text_lower for word in ["dying to", "kill for", "dead tired", "suicide mission"]
        ):
            return "metaphorical"
        ...
        if any(
            ...
            for phrase in ["how to help", "worried about", "friend is", "someone I know"]
        ):
    ...
    def analyze_confidence_distribution(
        self, y_prob: np.ndarray, y_true: np.ndarray, y_pred: np.ndarray
    ) -> Dict:
    ...
    def _calculate_calibration(
        self, y_prob: np.ndarray, y_true: np.ndarray, n_bins: int = 10
    ) -> Dict:
        ...
        ece = sum(d["bin_count"] / total_samples * d["calibration_error"] for d in calibration_data)

        return {"bins": calibration_data, "expected_calibration_error": ece}
    ...
    def create_visualizations(
        ...
        if y_prob is not None:
            ...
            correct_mask = y_true == y_pred
            ax3.hist(
                [y_prob[correct_mask], y_prob[~correct_mask]],
                bins=20,
                alpha=0.7,
            ...
            ax4.set_title(
                f'Calibration Plot (ECE={calibration["expected_calibration_error"]:.3f})',
                fontweight="bold",
            )
        ...
        fp_categories = Counter(
            [e.get("category", "unknown") for e in error_analysis["false_positives"]]
        )
        if fp_categories:
            ax5.pie(
                fp_categories.values(),
                labels=fp_categories.keys(),
                autopct="%1.1f%%",
                startangle=90,
            )
        ...
        fn_categories = Counter(
            [e.get("category", "unknown") for e in error_analysis["false_negatives"]]
        )
        if fn_categories:
            ax6.pie(
                fn_categories.values(),
                labels=fn_categories.keys(),
                autopct="%1.1f%%",
                startangle=90,
            )
        ...
        ax12.text(
            0.1, 0.9, summary_text, fontsize=11, verticalalignment="top", fontfamily="monospace"
        )
    ...
    def generate_report(
        self, error_analysis: Dict, confidence_analysis: Dict, text_analysis: Dict
    ) -> str:
...
"""
    ...
    def _generate_recommendations(
        self, error_analysis: Dict, confidence_analysis: Dict
    ) -> List[str]:
        """Generate recommendations based on error analysis."""
        ...
        total_errors = len(error_analysis["false_positives"]) + len(
        ...
        if not recommendations:
            recommendations.append(
                "Model performance is satisfactory. Consider A/B testing in production"
            )

        return recommendations

    def save_analysis(
        self, error_analysis: Dict, confidence_analysis: Dict, text_analysis: Dict, report: str
    ):
