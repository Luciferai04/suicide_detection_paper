    ...
    out = pd.DataFrame(
        {
            ...
            .astype(str)
            .str.lower()
            .map({"suicidal": 1, "1": 1, "non-suicidal": 0, "0": 0})
            .fillna(df[label_col])
            .astype(int),
        }
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Prepare Mendeley datasets: merge, anonymize, clean, split"
    )
    ...
    ap.add_argument(
        ...
        nargs="*",
        default=[],
        help="Optional quasi-identifier columns to check k-anonymity (if present)",
    )
    ...
    if not frames:
        raise SystemExit(
            "No usable Mendeley files found. Ensure downloads are placed under data/mendeley."
        )
