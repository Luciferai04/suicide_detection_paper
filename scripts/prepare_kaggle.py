...
def main():
    ap = argparse.ArgumentParser(
        description="Prepare Kaggle SuicideWatch dataset: anonymize, clean, split"
    )
    ...
    ap.add_argument(
        ...
        nargs="*",
        default=[],
        help="Optional quasi-identifier columns to check k-anonymity (e.g., age gender)",
    )
    ...
    if text_col is None:
        ...
        if "title" in cols_lower and ("post" in cols_lower or "body" in cols_lower):
            ...
            raise ValueError(
                f"Could not find text column in Kaggle CSV. Columns: {list(df.columns)[:10]} ..."
            )
    label_col = None
