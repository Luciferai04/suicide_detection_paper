...
class LiteratureBaselines:
    ...
    def plot_comparison(self, save_path: Optional[str] = None):
        ...
        # Color mapping
        colors = ["#FF6B6B" if t == "Our Work" else "#4ECDC4" for t in plot_df["Type"]]

        # 1. F1-Score comparison
        ax = axes[0, 0]
        plot_df_sorted = plot_df.sort_values("F1-Score")
        ax.barh(
            range(len(plot_df_sorted)),
            plot_df_sorted["F1-Score"],
            color=[colors[i] for i in plot_df_sorted.index],
        )
        ax.set_yticks(range(len(plot_df_sorted)))
        ax.set_yticklabels(
            [f"{m[:30]}..." if len(m) > 30 else m for m in plot_df_sorted["Model"]], fontsize=8
        )
        ...
        ax.barh(
            range(len(auc_df)),
            auc_df["AUC-ROC"],
            color=["#FF6B6B" if t == "Our Work" else "#4ECDC4" for t in auc_df["Type"]],
        )
        ax.set_yticks(range(len(auc_df)))
        ax.set_yticklabels(
            [f"{m[:30]}..." if len(m) > 30 else m for m in auc_df["Model"]], fontsize=8
        )
