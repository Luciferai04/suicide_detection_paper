...
class AblationStudy:
    ...
    def run_experiment(self, experiment: Dict) -> Dict:
        ...
        try:
            ...
            return {"experiment": experiment["name"], "success": False, "error": "Timeout"}
        except Exception as e:
            print(f"❌ Error: {experiment['name']} - {str(e)}")
            return {"experiment": experiment["name"], "success": False, "error": str(e)}
    ...
    def plot_ablation_results(self):
        ...
        if not impact_df.empty:
            ...
            ax.set_yticklabels(
                [f"{a[:30]}..." if len(a) > 30 else a for a in impact_sorted["Ablation"]],
                fontsize=8,
            )
        ...
        ax.bar(
            ...
            top_ablations["F1-Score"],
            width,
            label="F1-Score",
            alpha=0.7,
        )
        ax.bar(
            [i + width / 2 for i in x], top_ablations["AUC-ROC"], width, label="AUC-ROC", alpha=0.7
        )
        ax.set_xticks(x)
        ax.set_xticklabels(
            [e[:15] for e in top_ablations["Experiment"]], rotation=45, ha="right", fontsize=8
        )
        ax.set_ylabel("Score")
