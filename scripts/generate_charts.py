...
class ChartGenerator:
    def __init__(self):
        ...
        self.model_markers = {"bert": "o", "bilstm": "s", "svm": "^"}
    ...
    def create_accuracy_comparison(self, results: List[Dict]):
        ...
        for i, device in enumerate(devices):
            ...
            accuracies = [
                (
                    device_data[device_data["Model"] == model]["Accuracy"].values[0]
                    if len(device_data[device_data["Model"] == model]) > 0
                    else 0
                )
                for model in models
            ]
            ...
            bars = ax.bar(x + i * width, accuracies, width, label=device, color=color, alpha=0.8)
        ...
        plt.tight_layout()
        save_path = (
            self.plots_dir / f"accuracy_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
    ...
    def create_performance_speedup_chart(self, results: List[Dict]):
        ...
        pivot_df.plot(
            kind="bar",
            ax=ax,
            color=[self.device_colors.get(d.lower(), "#888888") for d in pivot_df.columns],
        )
        ...
        plt.tight_layout()
        save_path = (
            self.plots_dir / f"speedup_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def create_metrics_heatmap(self, results: List[Dict]):
        ...
        plt.tight_layout()
        save_path = (
            self.plots_dir / f"metrics_heatmap_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def create_device_comparison_radar(self, results: List[Dict]):
        ...
        for idx, model_type in enumerate(model_types):
            ...
            ax.set_ylim(0, 1)
            ax.set_title(
                f"{model_type.upper()} Performance", fontsize=12, fontweight="bold", pad=20
            )
            ax.legend(loc="upper right", bbox_to_anchor=(1.3, 1.1))
            ax.grid(True)

        plt.suptitle(
            "Device Performance Comparison (Radar Charts)", fontsize=14, fontweight="bold", y=1.05
        )
        plt.tight_layout()

        save_path = (
            self.plots_dir / f"radar_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def create_training_time_comparison(self, results: List[Dict]):
        ...
        for i, device in enumerate(devices):
            ...
            times = [
                (
                    device_data[device_data["Model"] == model]["Time (minutes)"].values[0]
                    if len(device_data[device_data["Model"] == model]) > 0
                    else 0
                )
                for model in models
            ...
            color = self.device_colors.get(device.lower(), "#888888")
            bars = ax.bar(x + i * width, times, width, label=device, color=color, alpha=0.8)
        ...
        save_path = (
            self.plots_dir
            / f"training_time_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        )
        plt.savefig(save_path, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def generate_all_charts(self):
        ...
        with open(chart_manifest, "w") as f:
            json.dump(
                {"generated": datetime.now().isoformat(), "charts": [str(p) for p in chart_paths]},
                f,
                indent=2,
            )
