...
def main():
    ...
    ap.add_argument(
        "--y_prob_a", required=True, help="Path to y_prob for model A (npy or JSON list)"
    )
    ap.add_argument(
        "--y_prob_b", required=True, help="Path to y_prob for model B (npy or JSON list)"
    )
    ...
    f1_a, ci_f1_a = bootstrap_ci(
        lambda y, p: f1_score(y, (p >= args.threshold).astype(int)), y_true, y_prob_a
    )
    f1_b, ci_f1_b = bootstrap_ci(
        lambda y, p: f1_score(y, (p >= args.threshold).astype(int)), y_true, y_prob_b
    )
