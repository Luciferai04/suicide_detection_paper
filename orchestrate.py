...
class TrainingOrchestrator:
    ...
    def start_training(self, models=['bert', 'bilstm', 'svm'], device='mps'):
        ...
        for model in models:
            ...
            cmd = [
                ...
                "--model", model,
                "--dataset", "kaggle",
                "--output_dir", f"results/model_outputs/{model}_{device}",
    ...
    def start_monitoring(self):
        ...
        with open(log_file, 'w') as log:
            process = subprocess.Popen(
                ["python", "scripts/auto_collect_enhanced.py"],
                stdout=log,
                stderr=subprocess.STDOUT
            )
    ...
    def wait_for_completion(self):
        ...
        try:
            while True:
                ...
                training_pids = [pid for name, pid in self.processes.items() 
                ...
                print(f"\r   Status: {alive_count}/{len(training_pids)} models still training "
                      f"(elapsed: {elapsed:.1f} min)", end='', flush=True)
...
def main():
    ...
    parser.add_argument('--mode', choices=['full', 'train', 'monitor', 'dashboard'],
                       default='full', help='Orchestration mode')
    parser.add_argument('--models', nargs='+', 
                       default=['bert', 'bilstm', 'svm'],
