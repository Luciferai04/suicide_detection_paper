...
class TrainingMonitor:
    ...
    def tail_log(self, logfile: str, lines: int = 10) -> str:
        ...
        try:
            result = subprocess.run(
                ["tail", "-n", str(lines), logfile], capture_output=True, text=True
            )
            if result.returncode == 0:
    ...
    def print_status_update(self):
        ...
        for model_name in ["svm", "bilstm", "bert"]:
            ...
            if status["status"] == "completed":
                ...
                print(
                    f"   💻 CPU: {stats['cpu_percent']}% | Memory: {stats['memory_percent']}% | Runtime: {stats['runtime']}"
                )
                print(
                    f"   ⏱️  Elapsed: {status['elapsed_minutes']}min | ETA: {status['estimated_completion']}"
                )
                ...
                if model_name == "bert" and "progress" in progress:
                    ...
                    if p["steps"] > 0:
                        print(
                            f"   📈 Progress: {p['percentage']:.1f}% ({p['steps']:,}/{p['total_steps']:,}) | Speed: {p['speed']}"
                        )

                    if "metrics" in progress and progress["metrics"]:
                        m = progress["metrics"]
                        print(
                            f"   📊 Loss: {m.get('loss', 'N/A')} | LR: {m.get('learning_rate', 'N/A'):.2e} | Epoch: {m.get('epoch', 'N/A')}"
                        )

                elif model_name == "svm":
                    print(
                        f"   📊 {progress.get('phase', 'Processing')}: {progress.get('details', 'In progress')}"
                    )

                elif model_name == "bilstm":
                    print(
                        f"   📊 {progress.get('phase', 'Training')}: {progress.get('details', 'In progress')}"
                    )
